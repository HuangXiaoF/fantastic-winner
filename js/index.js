...
Snake.prototype.init=function(){
    ...
    snakeBody2.create();
    this.tail=snakeBody2; //把蛇尾 的信息存起来
...
Snake.prototype.strategies={
    move:function(format){  //这个参数用于决定要不要删除最后一个方块（蛇尾）。当传了这个参数，代表要做的事情是吃
        //创建一个新身体（在旧蛇头的位置）
...
function createFood(){
    ...
    var include=true; //循环跳出的条件，true表示食物的坐标在蛇身上。（需要继续循环），false表示食物的坐标不在蛇身上，跳出循环
    while(include){
  ...
  food.pos=[x,y]; //存储一下生成食物的坐标，用于跟渗透要走的下一个点做对比
